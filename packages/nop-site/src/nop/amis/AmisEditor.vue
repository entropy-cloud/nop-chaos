...
import { fetchPageSource, rollbackPageSource, savePageSource, deletePageCache } from '../api';
import { ajaxFetch } from '../core';
...
function startFetch() {
  ...
  fetched = true
  return fetchPageSource(props.path, true).then((page) => {
    postMsg({
        type: 'toast',
        level: 'info',
        message: '页面加载成功'
